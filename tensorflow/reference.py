"""
!python3 reference.py --train-file ./namuwikitext_20200302.dev --dev-file ./namuwikitext_20200302.dev --training-config ./config.json --char-file ./chars-4996
"""
...
def sparse_categorical_crossentropy_with_ignore(y_true, y_pred, from_logits=False, axis=-1, ignore_id=-1):
    positions = tf.where(y_true != ignore_id)   # ignore_id과 같지않은 인덱스 반환
...
def main():
    ...
    with open(args.training_config, encoding='utf-8-sig') as f:
        config = json.load(f)

    with open(args.char_file, encoding='utf-8-sig') as f:
        content = f.read()
